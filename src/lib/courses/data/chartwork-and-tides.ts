      ...
      lessons: [
        {
          ...
          body: [
            ...
            },
            {
              type: "callout",
              tone: "note",
              title: "墨卡托直线 vs 大圆：纽约到伦敦差多少",
              body:
                "在墨卡托海图上，纽约 (40°43′N 074°W) 到伦敦 (51°30′N 000°W) 画一条直线（rhumb line）约 3450 海里，全程罗航向保持在 073° 附近；地球表面真正的最短路径（大圆 / Great Circle）只要约 3000 海里，但航向要从纽约的 051° 一路漂到伦敦的 105°，舵手每隔几小时就要重新算航向。450 海里 ≈ 跨大西洋多开 4 天——为什么 Day Skipper 还是选 rhumb line？因为近岸 50 海里航段里，大圆与等角航线差距 < 0.1 海里，可以忽略；只有跨洋时才值得用大圆，且分段画成多条 rhumb line 近似。墨卡托用「方向不变形」换「距离稍长」，对近岸船长是赚的。",
            },
            {
              type: "definition",
        ...
        },
        {
          ...
          body: [
            ...
            {
              type: "heading",
              text: "3 道题速查表：每行 4 列分步展开",
            },
            {
              type: "paragraph",
              text: "下面这张表把三种最常见的 CADET 题型——T→C、C→T、含年变率——按 4 行展开，每一列对应一个等号阶段。复习时把表盖住一列、自己填一遍，比读 10 遍段落记得牢。",
            },
            {
              type: "table",
              headers: ["题型", "起点", "第 1 次修正", "第 2 次修正"],
              rows: [
                ["T→C（最常考）：T 045°，V 3°W，D +2°E", "T = 045°", "T 045° + 3°（反向 W 加）→ M = 048°", "M 048° − 2°（反向 E 减）→ C = 046°"],
                ["C→T（取方位）：C 218°，V 5°W，D −3°W", "C = 218°", "C 218° − 3°（正向 W 减）→ M = 215°", "M 215° − 5°（正向 W 减）→ T = 210°"],
                ["含年变率：2020 标 4°20′W (8′E/yr)，2026 年 T 270°，D +3°E", "先更新 V：6 年 × 8′E = 48′E，4°20′W − 0°48′ = 3°32′W ≈ 3.5°W", "T 270° + 3.5°（反向 W 加）→ M = 273.5°", "M 273.5° − 3°（反向 E 减）→ C ≈ 270°"],
              ],
            },
            {
              type: "callout",
              ...
              title: "符号速记：正向 vs 反向",
              body:
                "正向（C→M→T，「化罗为真」）：W 减、E 加。反向（T→M→C，「化真为罗」，舵手实际要按的）：W 加、E 减。表格里每一步的符号都验证了这条规则——遇到 W 时，正反向加减号相反；遇到 E 时也相反。背不下来就把上面的表抄到笔记本扉页，做题时翻一下。",
            },
            {
              type: "paragraph",
      ...
      lessons: [
        ...
        },
        {
          ...
          body: [
            ...
            {
              type: "heading",
              text: "3 步分图：在纸海图上一步一步走",
            },
            {
              type: "paragraph",
              text: "上面那段文字一次性说完了三个动作，但实操时是分成 3 步逐张画的。下面把每一步独立列出来，配上「你的铅笔在做什么」——下次拿纸海图时，对着这 3 步走一遍，CTS 三角形就立得起来。",
            },
            {
              type: "list",
              ordered: true,
              items: [
                "第 1 步：画潮流向量。把 A 点（起航位置）当原点，沿当小时潮流的 set 方向（如 045° T）画一条直线，长度按 1 小时的 drift 取（如 2.0 节 = 2.0 nm）。终点标 T。这是「假设你不动、只有潮流推你 1 小时」会被带到的位置。",
                "第 2 步：画船速弧。把分规张开到「1 小时船速」距离（如 5.5 节 = 5.5 nm），针尖扎在 T 点，画一段弧线，使弧线扫过 A→B 目标线。弧与目标线的交点标 X。这是「从 T 出发以船速朝任何方向开 1 小时」能到达的所有可能位置中、与目标线相交的那一点。",
                "第 3 步：读 CTS。用量角器对准 T 点，量出 T→X 这一段向量的真方位（如 088° T）——这就是 CTS，舵手要让船头朝向的方向。A→X 这段连线则是你 1 小时后的实际位移（包含潮流推送），它的方向是 SOG track、长度除以 1 小时就是 SOG。",
              ],
            },
            {
              type: "callout",
              tone: "tip",
              title: "做完 3 步要立刻 sanity-check",
              body:
                "画完 T→X 向量、读出 CTS 后，问自己一句：「CTS 比目标方位偏向哪一侧？」答案应该是「偏向潮流来源一侧」。例如目标 080° T、潮流 045° T（来自西南、推船向东北），CTS 应该比 080° 大（偏南/偏潮流的反方向）——本题算得 088° T，符合。如果你算出 CTS 比目标方位偏向「潮流去向」一侧（如 072°），你一定把向量画反了——重画。",
            },
            {
              type: "practice",
              prompt:
                "尝试一道：A→B 真方位 120° T，距离 6 nm。潮流 060° T / 1.5 节，船速 4.5 节。① 在纸上画出 3 步分图（先画潮流向量 0→1.5 nm 沿 060°，再画船速弧 1.5 nm 处张开 4.5 nm，交目标线）；② 量出 CTS 真方位；③ 量出 A→X 实际位移长度，得到 SMG。提示：潮流朝东北推、目标在东南偏南——CTS 应略偏北（数值上比 120° 小 10°–15°），SMG 应比船速 4.5 节略大或略小？自己想一下。",
              hint:
                "潮流向量与目标方向夹角约 60°——既不是完全顺向也不是完全横向，主要是横推。CTS 应该向「上潮流方向」偏 15° 左右，约 105°–108° T。SMG 约 4.5 节附近，可能稍大（因为潮流有一点顺向分量）。算完后用 cos/sin 复核一次。",
            },
            {
              type: "heading",
              text: "风压角（Leeway）",
      ...
      lessons: [
        ...
        },
        {
          ...
          body: [
            ...
            {
              type: "paragraph",
              text: "下面把整个过程拆成 5 个小步骤，每一步只算一件事——别再塞一个长段落里。",
            },
            {
              type: "table",
              headers: ["步骤", "在算什么", "数值"],
              rows: [
                ["1. 要求的潮高", "实际水深 ≥ 吃水 + UKC = 2.2 + 0.6 = 2.8 m；海图水深 0.5 m；所求潮高 ≥ 2.8 − 0.5", "潮高 ≥ 2.3 m"],
                ["2. 需要涨多少", "潮高从 LW 高度 0.4 m 涨到 2.3 m", "需涨 1.9 m"],
                ["3. Range 与「12 分之几」", "Range = HW − LW = 4.8 − 0.4 = 4.4 m；需涨 1.9 m / 4.4 m × 12", "约 5.2 / 12"],
                ["4. 落在第几小时（涨潮序列 1-2-3-3-2-1，累计 1/3/6/9/11/12）", "5.2/12 落在第 3 小时末 (6/12) 之前一点——但实际是第 4 小时内的早段（6/12 → 9/12 的中间偏前）", "第 4 小时内，已过 (5.2−3)/3 ≈ 0.73 → 不对：正确读法是 5.2/12 比 6/12 小，所以落在第 3 小时内的末段；第 3 小时累计从 3/12（前 2 小时末）涨到 6/12，跨度 3/12，要补 5.2/12 − 3/12 = 2.2/12，占该小时的 2.2/3 ≈ 0.73"],
                ["5. 最终时刻", "LW 0800 + 2 小时（前两小时已完成）+ 0.73 小时 ≈ 2h 44min", "约 1044——再加 UKC 安全余量推荐取 1100"],
              ],
            },
            {
              type: "callout",
              ...
              title: "线性插值再验一次",
              body:
                "上面表第 4 步的算法是「第 N 小时内按分钟均匀插值」——这本身是把已经离散化的 12 分之 1 法则再线性化一次，精度对 Day Skipper 足够。如果想再精确一点，直接读 ATT 当地标准港的 tidal curve 图：在曲线上找到「LW + (1.9/4.4 × 6h) = 2h 35min」对应的时刻，结果会接近 1035–1100。两种方法都给到「11 点左右最早能进」的同一个答案。",
            },
            {
              type: "callout",
              tone: "warn",
      ...
      lessons: [
        ...
        },
        {
          ...
          body: [
            ...
            {
              type: "paragraph",
              text: "为了让累加过程一目了然，下面这张表把每小时拆开——每一行是当小时的「船速向量 east/north」和「潮流向量 east/north」，最后两行是累计。把它和上面段落里的数字对照，能看清「东向 17.9 - 1.88 = 16.0 nm」「北向 -1.6 - 2.97 = -4.57 nm」分别是怎么算出来的。",
            },
            {
              type: "table",
              headers: ["小时", "船速 east (nm)", "船速 north (nm)", "潮流 set/drift", "潮流 east (nm)", "潮流 north (nm)"],
              rows: [
                ["第 1 小时（P 海域）", "6 × sin 95° ≈ +5.98", "6 × cos 95° ≈ −0.52", "180° / 1.5", "1.5 × sin 180° = 0.00", "1.5 × cos 180° = −1.50"],
                ["第 2 小时（P 海域）", "+5.98", "−0.52", "200° / 1.8", "1.8 × sin 200° ≈ −0.62", "1.8 × cos 200° ≈ −1.69"],
                ["第 3 小时（Q 海域）", "+5.98", "−0.52", "280° / 1.3", "1.3 × sin 280° ≈ −1.28", "1.3 × cos 280° ≈ +0.23"],
                ["累计船速向量（DR）", "+17.94", "−1.56", "—", "—", "—"],
                ["累计潮流向量", "—", "—", "—", "−1.90", "−2.96"],
                ["EP = DR + 潮流（A 相对位移）", "+16.04 (东)", "−4.52 (南)", "—", "—", "—"],
              ],
            },
            {
              type: "paragraph",
